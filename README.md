...
Detecting fraudulent bank account applications at the point of submission using XGBoost with a cost-optimized decision threshold and SHAP-based explainability.

**ROC-AUC: 0.891 | Recall: 78.3% | Gini: 0.783 | KS: 0.623 | PR-AUC: 0.189 | Dataset: 1,000,000 applications**
...
|---|---|---|---|---|---|
| Random Forest (baseline) | 0.856 | - | 0.506 | 0.712 | - |
| XGBoost (scale_pos_weight) | **0.891** | **0.189** | **0.783** | **0.783** | **0.623** |
| XGBoost (SMOTE) | 0.878 | - | 0.761 | 0.756 | - |

XGBoost metrics are at the cost-based threshold (51.87%). RF metrics are at its own 5% FPR threshold.
...
An interactive scoring application that evaluates a single application in real time. The model scores the application and generates a SHAP waterfall explaining which features drove the decision.

```bash
streamlit run app/streamlit_app.py
```

Risk bands use the cost-based threshold (51.87%):
- Score below 30%: LOW
- Score between 30% and 51.87%: MEDIUM
- Score at or above 51.87%: HIGH
...
# 4. Run the demo from the project root
cd ../..
streamlit run app/streamlit_app.py
```
